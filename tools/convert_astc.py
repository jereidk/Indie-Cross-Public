...
def main() -> None:
    ...
    # Filter out oversized sprites (requires Pillow). Excludes anything wider
    # or taller than max_size -- large source textures are exactly what GPU
    # texture-size limits and driver quirks on lower-end Android devices trip
    # over, and this codebase's own SystemMonitor already flags >4096px
    # textures at runtime as a problem to fix. 0/unset disables the filter.
    max_size = config.get("max_size", 0)
    if max_size > 0:
        if not HAS_PIL:
            print("WARNING: Pillow not installed — max_size filter disabled. "
                  "pip install Pillow to exclude oversized sprites.")
        else:
            filtered = []
            oversized = []
            for f in png_files:
                try:
                    ...
                    if w <= max_size and h <= max_size:
                        ...
                        oversized.append((f, w, h))
                except Exception:
                    filtered.append(f)  # Include on error
            if oversized:
                print(f"Max-size filter (<={max_size}px): {len(png_files)} total → {len(filtered)} ({len(oversized)} excluded as oversized)")
                for f, w, h in oversized:
                    print(f"  ─  [oversized {w}x{h}]  {f}")
            png_files = filtered
