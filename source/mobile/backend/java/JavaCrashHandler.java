 ...
 *
 * Ported from NightmareVision-Android-Support's mobile.backend.java.JavaCrashHandler.
 */
public class JavaCrashHandler extends Extension implements Thread.UncaughtExceptionHandler {
     ...
     * system entirely.
     *
     * Needed because a CI-injected symbol file lands in the APK's real
     * assets/ folder AFTER Lime's own asset manifest (openfl.Assets,
     * generated during Lime's "update" phase at the very start of
     * "lime build android") has already been finalized. The file genuinely
     * ends up inside the packaged APK, but openfl.Assets.exists()/getContent()
     * only ever check that compile-time manifest -- never the real APK
     * contents -- so they silently never see it. AssetManager.open() reads
     * the real, current assets/ contents directly, sidestepping that
     * mismatch.
     *
    ...
    /**
     * Appends a line to game.log (Haxe's GameLogger, same file this class
     * already writes crash.log/trace dumps next to), so non-fatal Java-side
     * log calls elsewhere show up alongside everything Haxe already logs
     * there instead of only ever reaching Logcat -- which needs adb, not
     * available to most players reporting a bug.
     *
     * Shared here (keyed off sCrashLogPath, already known once install() has
     * run) rather than each caller deriving its own path. Only appends if
     * game.log already exists -- GameLogger.init() (called as early as
     * possible in Caching.create(), right before this class's install() is
     * called) has already had its chance to create it for this session.
     */
     ...
     * retention cap. Two consequences of only ever asking for the single
     * latest entry (maxNum=1) and overwriting one fixed filename every time
     * would be:
     *
     *   1. Two crashes in a row, before the user extracts each trace, would
     *      mean the second overwrote the first's saved file -- lost for
     *      good, even though Android itself still had both.
     *   2. Since Android never clears its own history, re-running the same
     *      maxNum=1 query on every subsequent launch would keep
     *      re-surfacing (and re-overwriting the trace file for) the SAME
     *      old crash indefinitely, until enough newer exits pushed it out
     *      of the OS's own retention.
     *
     * Avoided by asking for the full available history each time (maxNum=0),
     * saving every new exit's trace under its own pid+timestamp filename,
     * and persisting the newest timestamp already processed so a later
     * launch only reports genuinely new exits.
     *
    ...
    public static String readPreviousNativeCrash() {
        ...
        try {
            ...
            // against the wrong .so -- a class of misattribution worth
            // avoiding, since nearest-symbol guessing on unrelated code is
            ...
            for (Object info : exits) {
                ...
                sb.append("Crash detected (previous session)\n\n");
                sb.append("Date: ").append(formatTimestamp(timestamp)).append("\n");
                sb.append("Type: ").append(reasonLabel(reason)).append("\n");
                if (desc != null && !desc.toString().isEmpty())
                    sb.append("Description: ").append(desc).append("\n");
                sb.append("Process state: ").append(importanceLabel(importance)).append("\n");
                sb.append("Exit code: ").append(status).append("\n");
                if (processName != null && !processName.isEmpty())
                    sb.append("Process: ").append(processName).append("\n");
                if (pss >= 0 || rss >= 0)
                    sb.append("Memory PSS/RSS: ").append(pss).append("KB / ").append(rss).append("KB\n");
                ...
                if ((reason == 5 || reason == 6) && Build.VERSION.SDK_INT >= 31) {
                    ...
                    if (tracePath != null) sb.append("Trace saved to: ").append(tracePath).append("\n");
                }
            }
            ...
            String header = "";
            if (prevBuildInfo != null) header += "Build (previous session): " + prevBuildInfo + "\n\n";
            if (newCount > 1)
                header += "(" + newCount + " abnormal exits detected since the last launch"
                    + " -- if the app was updated between them, the build above"
                    + " only certainly applies to the most recent one)\n\n";
     ...
     *
     * Named per pid+timestamp (not a single fixed filename) so a second
     * crash before the user has grabbed the first one's trace doesn't
     ...
     * each other -- but that also means they never get cleaned up on their
    ...
    private static String reasonLabel(int reason) {
        switch (reason) {
            case 2:  return "SIGNAL (SIGKILL / system)";
